...
- Реальний зміст лише з `--backend claude` (`ANTHROPIC_API_KEY`); `mock` — ехо (тести/офлайн-каркас).
- `.gitignore` не парситься — відкладено (майбутній `--gitignore` + `pathspec`).
- Великі репо обрізаються `--max-chars` (single-shot Claude на документ, як ADR-0002).
